...
class XMPPClient {
			...
			callback: () => {
				...
				const filteredInbox = inboxMessages.filter((message) => {
					const inboxMessageId =
						sharedConfig.useStore.getState().chatsRegistry[message.roomId]?.inboxMessageId;
					const cleanHistoryDate =
						sharedConfig.useStore.getState().rooms[message.roomId]?.userSettings?.clearedAt;
					return (
				...
				const { setInboxMessages, addFastening, setLastMessage } = sharedConfig.useStore.getState();
								...
								if (textOrConfig) {
									...
									if (newLastMessage.type === MessageType.TEXT_MSG) {
										const lastFastening = getEditAndDeleteFasteningSelector(
											sharedConfig.useStore.getState(),
											fastening.roomId,
	...
	sendChatMessage(roomId: string, message: string): void {
		...
		sharedConfig.useStore.getState().setPlaceholderMessage({ roomId, id: uuid, text: message });
	...
	public requestMessages(
		...
		return new Promise((resolve, reject) => {
			if (!sharedConfig.useStore.getState().rooms[roomId]) {
				resolve(null);
				return;
			}

			const clearedAt = sharedConfig.useStore.getState().rooms[roomId]?.userSettings?.clearedAt;
			const startHistory =
				clearedAt ?? sharedConfig.useStore.getState().rooms[roomId]?.createdAt ?? 0;
	...
	requestHistory(roomId: string, endHistory: number, quantity = 50, unread = 0): void {
		if (!sharedConfig.useStore.getState().rooms[roomId]) return;
		const clearedAt = sharedConfig.useStore.getState().rooms[roomId].userSettings?.clearedAt;
		const startHistory = clearedAt ?? sharedConfig.useStore.getState().rooms[roomId].createdAt;
		const queryId = HistoryAccumulator.getNextId();
	...
	requestMessageSubjectOfReply(
		...
		if (!sharedConfig.useStore.getState().rooms[roomId]) return;
		const storeMessages = sharedConfig.useStore.getState().chatsRegistry[roomId]?.messages;
		const referenceMessage = find(
			storeMessages,
			(message) => message.id === messageSubjectOfReplyId && message.type === MessageType.TEXT_MSG
		) as TextMessage;
		if (referenceMessage) {
			sharedConfig.useStore.getState().setRepliedMessage(roomId, replyMessageId, referenceMessage);
		} else {
			const queryId = HistoryAccumulator.getNextId();
			const iq = $iq({ type: 'set', to: carbonizeMUC(roomId) })
				.c('query', { xmlns: Strophe.NS.MAM, queryid: queryId })
			...
			this.xmppConnection.send({
				...
				callback: () => {
					...
					const { setRepliedMessage } = sharedConfig.useStore.getState();
	...
	requestFullHistory(roomId: string, from?: number): void {
		if (!sharedConfig.useStore.getState().rooms[roomId]) return;
		const room = sharedConfig.useStore.getState().rooms[roomId];
		...
		return new Promise<void>((resolve, reject) => {
			...
			this.xmppConnection.send({
				...
				callback: () => {
					...
					sharedConfig.useStore.getState().setSearchResults(roomId, searchedMessages);
					resolve();
				},
				errorCallback: reject
			});
	...
	sendIsWriting(roomId: string): void {
		...
		if (sharedConfig.useStore.getState().rooms[roomId]?.placeholder) return;
	...
	readMessage(roomId: string, messageId: string): void {
		const message = find(
			sharedConfig.useStore.getState().chatsRegistry[roomId].messages,
	...
	private handleEditedPinnedMessage(roomId: string, fasteningMessage: MessageFastening): void {
		this.fetchMessageByStanzaId(roomId, fasteningMessage.originalStanzaId, (queryId) => {
			...
			sharedConfig.useStore.getState().setPinnedMessage(roomId, editedMessage);
		});
	}
	...
	requestPinnedMessageContent(roomId: string, pinnedMessageStanzaId: string): void {
		...
		if (existingMessage) {
			sharedConfig.useStore.getState().setPinnedMessage(roomId, existingMessage);
			return;
		}
		...
		this.fetchMessageByStanzaId(roomId, pinnedMessageStanzaId, (queryId) => {
			...
			if (message.type === MessageType.TEXT_MSG) {
				sharedConfig.useStore.getState().setPinnedMessage(roomId, message);
				return;
			}
