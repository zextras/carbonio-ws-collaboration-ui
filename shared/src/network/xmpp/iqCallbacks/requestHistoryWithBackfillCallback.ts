...
export function requestHistoryWithBackfillCallback(stanza: Element, queryId: string): void {
	...
	const gaps = detectGaps(
		sharedConfig.useStore.getState().chatsRegistry[roomId].messageRanges ?? []
	);
	store.enqueueBackfill(roomId, gaps);

	if (sharedConfig.useStore.getState().chatsRegistry[roomId].backfillQueue.length > 0) {
		const request = sharedConfig.useStore.getState().chatsRegistry[roomId].backfillQueue[0];
