...
export function handleHistory(
	...
	sharedConfig.useStore.getState().addFastening(fasteningMessages);

	// Store history messages on store updating the history of the room
	sharedConfig.useStore.getState().updateHistory(roomId, storeMessages);
	...
	if (historyMessages.length > 0) {
		...
		sharedConfig.useStore.getState().addMessageRange(roomId, rangeInfo);
	}
