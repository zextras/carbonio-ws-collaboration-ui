...
describe('WebSocketClient', () => {
	...
	test('onopen event is handled resetting reconnectionTine, setting ping and setting ws status into store', () => {
		...
		expect(sharedConfig.useStore.getState().connections.status.websocket).toBeTruthy();
	});
	...
	test('onclose event is handled setting ws status into store', () => {
		...
		wsClient.connect();
		wsClient._onClose();
		expect(sharedConfig.useStore.getState().connections.status.websocket).toBeFalsy();
	});
	...
	test('onmessage event is handled with a text message', () => {
		...
		wsClient._onMessage({ data: JSON.stringify(message) } as MessageEvent);
		expect(sharedConfig.useStore.getState().session.queueId).toBe('queueId');
	});
	...
	test('Set apiVersion on store when selected protocol is different from session apiVersion', () => {
		const wsClient = new WebSocketClient();
		sharedConfig.useStore.getState().setApiVersion('1.6.0');
		wsClient.connect();
		wsClient._webSocket = {
			...wsClient._webSocket,
			protocol: '1.6.1'
		...
		wsClient._onOpen();
		expect(sharedConfig.useStore.getState().session.apiVersion).toBe('1.6.1');
	});
});
