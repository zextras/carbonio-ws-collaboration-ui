...
import { pauseWritingMessage, startWritingMessage } from '../xmppMessageExamples';

const composingStanza = (roomId: string, from: string): Element => {
...
const pausedStanza = (roomId: string, from: string): Element => {
	const parser = new DOMParser();
...
const mockedRoom = createMockRoom({ participants: [user0] });
