 ...
 * */
export function requestHistoryCallback(
	stanza: Element,
	queryId: string,
	unread?: number,
	...
	// Avoid setting history fully loaded if backfill is enabled
	if (!enableBackfill && (isHistoryFullyLoaded || size(historyMessages) === 0)) {
	...
	if (isHistoryFullyLoaded && !historyIsBeenCleared && !enableBackfill) {
