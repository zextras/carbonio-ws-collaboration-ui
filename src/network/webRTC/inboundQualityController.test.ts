...
test('drops on a single congested tick', () => {
...
test('congestion drops FRAMERATE first: rung 5 -> 4 keeps the substream, cuts temporal', () => {
	const s = drop(initialQualityState(), 1);
	expect(s.rung).toBe(4);
	expect(s.changeSubstream).toBe(2);
...
test('turns OFF at the very bottom (sub0, base) on a single congested tick', () => {
...
test('escalating patience: a failed climb doubles the streak needed at that boundary', () => {
	...
	expect(s.rung).toBe(1);
	s = decideQuality(s, congested); // drop 1 -> 0, within OBSERVE -> failCount[0] -> 1
	expect(s.rung).toBe(0);
...
test('dead band (2-5%) holds and resets the clean streak', () => {
	let s = initialQualityState(0);
	s = climb(s, 2);
	s = decideQuality(s, deadBand); // resets cleanStreak
	s = climb(s, 7);
