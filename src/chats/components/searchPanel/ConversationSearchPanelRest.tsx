...
const ConversationSearchPanelRest: FC<ConversationSearchPanelProps> = ({
	roomId,
	goToChatView
}) => {
	...
	const loadMore = useCallback(() => {
		...
		const beforeTimestamp = lastMessage.date ? new Date(lastMessage.date).toISOString() : undefined;
	...
	useEffect(() => {
		...
		if (!triggerElement || !scrollContainer) return undefined;
		...
		<Container>
			...
			<div
				...
				style={{
					...
					gap: '0.5rem',
					justifyContent:
						requestStatus === RequestStatus.SUCCESS && results && results.length > 0
							? 'flex-start'
							: 'center',
					alignItems: 'center'
				}}
...
export default ConversationSearchPanelRest;
