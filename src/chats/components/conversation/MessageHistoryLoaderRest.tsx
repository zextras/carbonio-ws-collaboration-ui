...
const PLACEHOLDER_ROOM_PREFIX = 'placeholder-';
...
export const HistoryLoaderBefore = ({
	...
	const handleLoadBefore = useCallback(() => {
		...
		if (roomId.startsWith(PLACEHOLDER_ROOM_PREFIX)) return;
		...
		hasMoreBefore,
		currentUserId,
		setHistoryLoadDisabled,
		setHasMoreBefore,
		setHistoryIsFullyLoaded
	]);
 ...
 */
const MessageHistoryLoaderRest = ({ roomId, messageListRef }: HistoryLoaderProps): ReactElement => {
	const intersectionObserverRef = useRef<IntersectionObserver>();
...
export default MessageHistoryLoaderRest;
